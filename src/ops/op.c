...
const tele_op_t *tele_ops[E_OP__LENGTH] = {
    ...
    // maths
    &op_ADD, &op_SUB, &op_MUL, &op_DIV, &op_MOD, &op_CNT, &op_RAND, &op_RND,
    &op_RRAND, &op_RRND, &op_R, &op_R_MIN, &op_R_MAX, &op_TOSS, &op_MIN,
    &op_MAX, &op_LIM, &op_WRAP, &op_WRP, &op_QT, &op_QT_S, &op_QT_CS, &op_QT_B,
    &op_AVG, &op_EQ, &op_NE, &op_LT, &op_GT, &op_LTE, &op_GTE, &op_NZ, &op_EZ,
    &op_RSH, &op_LSH, &op_LROT, &op_RROT, &op_EXP, &op_ABS, &op_SGN, &op_AND,
    &op_OR, &op_JI, &op_SCALE, &op_SCL, &op_N, &op_VN, &op_N_S, &op_N_C,
    &op_N_CS, &op_N_B, &op_N_BX, &op_V, &op_VV, &op_ER, &op_NR, &op_BPM,
    &op_BIT_OR, &op_BIT_AND, &op_BIT_NOT, &op_BIT_XOR, &op_BSET, &op_BGET,
    &op_BCLR, &op_BTOG, &op_BREV, &op_XOR, &op_CHAOS, &op_CHAOS_R,
